...
export async function POST(
  ...
  try {
    ...
    if (user.role !== Role.FACULTY && user.role !== Role.ADMIN) {
      return NextResponse.json(
        { success: false, error: "Forbidden", message: "Only faculty and administrators can publish assignments." },
        { status: 403 }
      );
    }

    const { id } = await params;
    const assignment = await AssignmentService.publishAssignment(user.id, user.role, id);

    return NextResponse.json({
      success: true,
      message: `Assignment "${assignment.title}" is now published and open for submissions.`,
      assignment,
    });
  } catch (error) {
