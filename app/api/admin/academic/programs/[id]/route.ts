...
import { updateProgramSchema } from "@/validators/academic.schema";
...
export async function PATCH(
  ...
  try {
    ...
    const program = await AcademicService.updateProgram(id, parsed.data, auth.user.id);
    return NextResponse.json({ success: true, program });
  } catch (err: unknown) {
...
export async function DELETE(
  ...
  const { id } = await context.params;
  try {
    const program = await AcademicService.deleteProgram(id, auth.user.id);
    return NextResponse.json({ success: true, program, message: "Program deactivated successfully" });
  } catch (err: unknown) {
